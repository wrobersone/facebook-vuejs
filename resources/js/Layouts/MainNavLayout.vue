...
<template>
    <div class="flex bg-white w-full fixed z-50 items-center justify-between h-[56px] shadow-xl border-b" id="MainNav">
        <div class="flex items-center justify-start w-[260px]" id="NavLeft">
            <Link class="pl-3 min-w-[55px]" href="/">
            ...
            <div class="flex items-center justify-center bg-[#EFF2F5] p-1 rounded-full h-[40px] ml-2">
                ...
                <input class="hidden border-none lg:block p-0 bg-[#EFF2F5] placeholder-[#64676B] ring-0 focus:ring-0" type="text" placeholder="Search Facebook">
            </div>
        </div>

        <div class="hidden items-center lg:flex ml-5 justify-center w-8/12 max-w-[600px]" id="NavCenter">
            <Link href="" class="w-full">
                <div class="flex items-center justify-center h-[48px] p-1 hover:bg-[#F2F2F2] w-full rounded-lg cursor-pointer" :class="$page.url === '/' ? 'mt-1.5' : '' ">
                    <div>
                        <Home class="mx-auto" :size="27" fillColor="#1A73E3" v-if="$page.url" />
                        <HomeOutline v-else class="mx-auto"  />
                    </div>
                </div>
        ...
        <div class="flex items-center justify-end w-2/12 mr-4">
            ...
            <div class="flex items-center justify-center relative">
                <button @click="showMenu = !showMenu">
                    <img class="cursor-pointer ml-1 rounded-full min-w-[40px] max-h-[40px]" :src="user.image" alt="">
                </button>
                <div v-if="showMenu" class="absolute bg-white shadow-xl top-10 right-0 w-[330px] rounded-lg p-1 border mt-1">
                    <Link :href="route('user.show', { id: user.id })" @click="showMenu = !showMenu">
                        <div class="flex items-center gap-3 hover:bg-gray-200 p-2 rounded-lg">
                            <img class="cursor-pointer ml-1 rounded-full min-w-[35px] max-h-[35px]" :src="user.image" alt="">
                            <span>{{ user.name }}</span>
                        </div>
                    </Link>
